...
/// 显示当前会话的上下文占用；悬停时提供 token 和账户限额详情。
struct UsageRingButton: View {
  ...
  var body: some View {
    if showWhenUnavailable || usage.map(hasVisibleUsage) == true {
      ...
      .accessibilityValue(usage.map(contextAccessibilityValue) ?? "Unavailable")
    }
  }
  ...
  private var contextPercent: String {
    guard let used = usage?.used, let size = usage?.size, size > 0 else { return "--" }
    return "\(Int((min(Double(used) / Double(size), 1) * 100).rounded()))%"
  }
  ...
  private var popoverContent: some View {
    VStack(alignment: .leading, spacing: 14) {
      if let usage {
        ...
        Text("No usage yet").font(.caption).foregroundStyle(.secondary)
      }
  ...
  private func hasVisibleUsage(_ usage: SessionUsage) -> Bool {
    usage.used != nil || usage.inputTokens != nil || usage.cachedInputTokens != nil || usage.outputTokens != nil
      || usage.reasoningOutputTokens != nil || usage.totalTokens != nil || usage.cost != nil
  }

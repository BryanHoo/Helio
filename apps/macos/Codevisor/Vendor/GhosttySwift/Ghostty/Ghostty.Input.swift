// CODEVISOR-PATCH: removed `import AppIntents` (AppEnum blocks removed below)
import Cocoa
import SwiftUI
import GhosttyKit
...
// CODEVISOR-PATCH: removed AppEnum conformance block (upstream Shortcuts/App Intents support; conflicts with Codevisor's default-MainActor isolation and Codevisor ships no App Intents)
...
// CODEVISOR-PATCH-BEGIN: removed `Ghostty.Input.Key: AppEnum` conformance block
// (upstream L1179-1316; Shortcuts/App Intents support conflicts with Codevisor's
// default-MainActor isolation and Codevisor ships no App Intents). The `allCases`
// member it provided is used by `Key.init?(keyCode:)`, so it is preserved below
// verbatim, minus the AppEnum conformance and display representations.

// CODEVISOR-PATCH-BEGIN (file-wide note): all `AppDelegate.logger` references were
// replaced with `Ghostty.logger` (14 sites; AppDelegate is not vendored). Larger
// patches are marked inline with CODEVISOR-PATCH markers.
// CODEVISOR-PATCH-END
import AppKit
import Combine
import SwiftUI
...
import GhosttyKit
// CODEVISOR-PATCH-BEGIN: explicit os import for Logger under MemberImportVisibility
// (upstream gets it transitively)
import os
// CODEVISOR-PATCH-END

extension Ghostty {
    /// The NSView implementation for a terminal surface.
    // CODEVISOR-PATCH: dropped Codable conformance (window-restoration support;
    // see removed MARK: Codable section near end of class)
    class SurfaceView: OSSurfaceView, Identifiable {
        ...
        init(_ app: ghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            // CODEVISOR-PATCH-BEGIN: app-wide config comes from CodevisorGhosttyApp,
            // not AppDelegate (upstream L226-230)
            self.derivedConfig = DerivedConfig(CodevisorGhosttyApp.shared.config)
            // CODEVISOR-PATCH-END
        ...
        private func localEventLeftMouseDown(_ event: NSEvent) -> NSEvent? {
            // CODEVISOR-PATCH: removed command-palette visibility guard (upstream
            // L635-641; BaseTerminalController/CommandPalette not vendored)
        ...
        override func mouseMoved(with event: NSEvent) {
            ...
            // CODEVISOR-PATCH: removed focus-follows-mouse block (upstream L1007-1015;
            // requires BaseTerminalController which is not vendored — Codevisor does
            // not offer focus-follows-mouse)
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            if let bindingFlags {
                ...
                // CODEVISOR-PATCH: removed the menu-item key-equivalent attempt
                // (upstream L1302-1310; AppDelegate.performGhosttyBindingMenuKeyEquivalent
                // is not vendored — Codevisor has no ghostty-managed menu). Bindings
                // fall through to keyDown, which encodes them via libghostty.
        ...
        override func menu(for event: NSEvent) -> NSMenu? {
            ...
            // CODEVISOR-PATCH-BEGIN: reduced context menu (upstream L1572-1593).
            // Removed: Split Right/Left/Down/Up (splits not supported),
            // Toggle Terminal Inspector (inspector UI not vendored),
            // Change Tab Title / Change Terminal Title (BaseTerminalController /
            // window-title plumbing not vendored). Kept: Copy, Paste, Select All,
        ...
        // CODEVISOR-PATCH: removed splitRight/Left/Down/Up @IBActions (upstream
        // L1681-1699; their context-menu items were removed — Codevisor does not
        // support ghostty-managed splits)
        ...
        // CODEVISOR-PATCH: removed Codable support (upstream L1791-1833, MARK: Codable —
        // CodingKeys, init(from:), encode(to:)). Window-restoration decoding depends on
        // AppDelegate + TerminalRestoreError, neither vendored; Codevisor owns session
        // persistence (TerminalSession keeps surfaces alive across panel re-parenting).
    }
}

...
import GhosttyKit
import SwiftUI
// CODEVISOR-PATCH-BEGIN: explicit Combine + os imports for ObservableObject
// synthesis and Logger under MemberImportVisibility (upstream gets them transitively)
import Combine
import os
// CODEVISOR-PATCH-END
...
#if canImport(AppKit)
            // CODEVISOR-PATCH: AppDelegate.logger -> Ghostty.logger (AppDelegate not vendored)
